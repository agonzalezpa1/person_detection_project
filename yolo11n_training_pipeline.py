...
model.to("cuda") # pour forcer à utiliser le GPU T4 sur Google Collab, à changer s'il faut utiliser un autre

# Entraîne le modèle avec le dataset sur Kaggle
total_epochs = 50 # nombre de epochs désiré
...
model.train(
    ...
    device = 0, # Utilisant le GPU T4 de Google Collab
)

# Retour sur le score F1 du modèle après l'entraînement
metrics = model.val()
